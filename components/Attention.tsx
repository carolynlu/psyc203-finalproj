...
// static lobe functions
const getLobeFunction = (lobeName: keyof typeof LOBE_CONFIG): { primary: string } => {
    switch (lobeName) {
        case "Frontal Lobe":
            return {
                primary: "Responsible for executive functions (planning, judgment, decision-making), attention regulation, personality, voluntary motor control.",
            };
        case "Parietal Lobe":
            return {
                primary: "Integrates sensory input (touch, temperature, pain), manages spatial awareness, crucial for shifting attention.",
            };
        case "Temporal Lobe":
            return {
                primary: "Primarily deals with auditory information (hearing), vital for memory formation (hippocampus), manages language comprehension.",
            };
        case "Occipital Lobe":
            return {
...
                const lobeInfo = getLobeFunction(region.name as keyof typeof LOBE_CONFIG);
                
                return (
...
                    </div>
                    <p className="text-lg text-slate-700 font-serif mb-6">{lobeInfo.primary}</p>
