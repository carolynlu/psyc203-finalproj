...
import type { Scenario, PerceptionMode, SensoryModality } from '../types';
import { Eye, BookOpen, Coffee, Search, Ear, Hand, Activity, Radio, Utensils, Music, Wind, Fingerprint, Sparkles, Pencil } from 'lucide-react';
...
const SCENARIOS: Scenario[] = [
  { id: '1', title: 'Finding a Face', description: 'Trying to find your friend in a busy restaurant.', icon: 'Search', modality: 'Visual' },
  { id: '2', title: 'Orchestra Concert', description: 'Listening to all the different instruments.', icon: 'Music', modality: 'Auditory' },
  { id: '3', title: 'Fresh Coffee', description: 'Identifying coffee by scent alone.', icon: 'Coffee', modality: 'Olfactory' },
  { id: '4', title: 'Reading Braille', description: 'Decoding text through finger sensitivity.', icon: 'Fingerprint', modality: 'Tactile' },
  { id: '5', title: 'Trying New Foods', description: 'Analyzing the flavors and spiciness of a new dish.', icon: 'Utensils', modality: 'Gustatory' },
];
...
const PERCEPTION_MODES = {
  BOTTOM_UP: 'Bottom-Up' as const,
  TOP_DOWN: 'Top-Down' as const,
};
  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-center p-6 max-w-6xl mx-auto overflow-hidden">
      ...
      <div className="w-full mb-6 text-center z-10 transition-all">
         <h2 className="text-4xl font-bold font-sans text-[#f4a261] mb-2">
          Stage 2: Perception
        </h2>
        <p className="text-slate-600 max-w-2xl mx-auto h-12 font-medium font-serif">
          The brain receives the chaotic data. Select a real-world scenario or generate your own.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full h-full min-h-[400px] z-10">
        ...
        <div className="space-y-4">
          <h3 className="text-xl font-bold font-sans text-slate-700 border-b border-[#f4a261]/40 pb-2 mb-4 flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
            {SCENARIOS.map((scenario) => (
              ...
              >
                <div className={`mb-3 p-2 rounded-xl inline-block transition-colors ${
                    selectedScenario?.id === scenario.id
                      ? 'bg-[#f4a261] text-white shadow-md'
                      : 'bg-slate-100 text-slate-600 group-hover:bg-[#f7b68a] group-hover:text-white'
                }`}>
                  <ScenarioIcon name={scenario.icon} className="w-6 h-6" />
                </div>
                <h4 className="font-bold text-slate-800 mb-1">{scenario.title}</h4>
                <div className="text-xs font-bold font-mono text-[#f4a261] mb-1 uppercase tracking-wider">{scenario.modality} Input</div>
            ...
            >
              {!isCustomSelected ? (
                <>
                  <div className="mb-3 p-2 rounded-xl inline-block bg-slate-100 text-slate-600 group-hover:bg-[#f7b68a] group-hover:text-white transition-colors">
                    <Sparkles className="w-6 h-6" />
                  </div>
                  <h4 className="font-bold text-slate-800 mb-1">Create Your Own</h4>
                  <div className="text-xs font-bold font-mono text-[#f4a261] mb-1 uppercase tracking-wider">Custom Input</div>
                </>
              ) : (
                <div className="flex flex-col gap-3 animate-fade-in">
                  <div className="flex items-center gap-2 text-[#e07b39] font-bold border-b border-[#f4a261]/40 pb-2">
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-slate-800 focus:outline-none focus:border-[#f4a261] focus:ring-1 focus:ring-[#f4a261] placeholder-slate-400"
                      autoFocus
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {(Object.keys(MODALITY_ICONS) as (keyof typeof MODALITY_ICONS)[]).map((m) => {
                        ...
                        return (
                          <button
                            ...
                            className={`p-2 rounded-lg border transition-all ${isActive ? 'bg-[#f4a261] border-[#e07b39] text-white shadow-md' : 'bg-slate-50 border-slate-200 text-slate-500 hover:bg-[#fff3e0] hover:text-[#f4a261]'}`}
                            title={m}
                          >
        ...
        <div className="flex flex-col space-y-6">
           <h3 className="text-xl font-bold text-slate-700 border-b border-[#f4a261]/40 pb-2 mb-4">
            2. Choose Processing Strategy
          </h3>
          
          <div className="flex gap-4">
            <button
              onClick={() => onSelectMode(PERCEPTION_MODES.BOTTOM_UP)}
              className={`flex-1 p-6 rounded-2xl border flex flex-col items-center justify-center gap-3 transition-all shadow-sm ${
                perceptionMode === PERCEPTION_MODES.BOTTOM_UP
                  ? 'bg-[#fff3e0] border-[#e07b39] shadow-[0_0_15px_rgba(244,162,97,0.2)] ring-1 ring-[#f4a261]'
                  : 'bg-white/60 border-slate-200 hover:bg-[#fff3e0] hover:border-[#f4a261]'
              }`}
            >
            ...
            <button
              onClick={() => onSelectMode(PERCEPTION_MODES.TOP_DOWN)}
              className={`flex-1 p-6 rounded-2xl border flex flex-col items-center justify-center gap-3 transition-all shadow-sm ${
                perceptionMode === PERCEPTION_MODES.TOP_DOWN
                  ? 'bg-[#fff3e0] border-[#e07b39] shadow-[0_0_15px_rgba(244,162,97,0.2)] ring-1 ring-[#f4a261]'
                  : 'bg-white/60 border-slate-200 hover:bg-[#fff3e0] hover:border-[#f4a261]'
              }`}
            >
          ...
          <div className="flex-grow flex items-end justify-center pt-8">
            <button
              ...
              className={`w-full py-4 rounded-2xl font-bold text-xl transition-all shadow-lg flex items-center justify-center gap-3 relative overflow-hidden ${
                !isProcessDisabled
                  ? 'bg-[#f4a261] hover:bg-[#f7b68a] text-white cursor-pointer transform hover:scale-[1.02] shadow-[#f4a261]/30'
